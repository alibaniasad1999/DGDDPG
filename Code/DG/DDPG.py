...
class MLPGaussianActor(Actor):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, output_activation=nn.Identity):
        ...
        self.mu_net = mlp([obs_dim] + list(hidden_sizes) + [act_dim], activation, output_activation)
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space,
        ...
        if isinstance(action_space, Box):
            self.pi = MLPGaussianActor(obs_dim, action_space.shape[0], hidden_sizes, activation, output_activation)
...
class ThreeBodyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        return 1000*self.state, reward, done, False, self.position, reward_2
...
class DDPGBuffer:
    """
    A buffer for storing trajectories experienced by a DDPG agent interacting
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    adv=self.adv_buf, logp=self.logp_buf, # add second player
                    act_2=self.act_buf_2, ret_2=self.ret_buf_2,
                    adv_2=self.adv_buf_2, logp_2=self.logp_buf_2)
...
class DDPG:
    def __init__(self, env, ac_kwargs=None, seed=0,
        ...
        self.second_player = False
        self.second_player_learning = False
        self.random_learning = True
        ...
        # add second player
        self.ac_2 = MLPActorCritic(self.env.observation_space, self.env.action_space, output_activation=nn.Tanh, **self.ac_kwargs)
        ...
        self.logger.log('\n1th player Number of parameters 😱😱😱: \t pi: %d, \t v: %d\n' % var_counts)
        # add second player
        var_counts_2 = tuple(count_vars(module) for module in [self.ac_2.pi, self.ac_2.v])
        self.logger.log('\n2th player Number of parameters 😱😱😱: \t pi: %d, \t v: %d\n' % var_counts_2)

        # Set up experience buffer
        self.local_steps_per_epoch = int(steps_per_epoch / num_procs())
        self.buf = DDPGBuffer(obs_dim, act_dim, self.local_steps_per_epoch, gamma, lam)
        ...
        self.logger.setup_pytorch_saver(self.ac_2)

    # Set up function for computing DDPG policy loss
    def compute_loss_pi(self, data, player_no='1'):
        if player_no == '1':
    ...
    def compute_loss_v(self, data, player_no='1'):
        if player_no == '1':
    ...
    def update(self):
        ...
        # Player 1 updates
        # Compute initial loss and other metrics for player 1
        pi_l_old, pi_info_old = self.compute_loss_pi(data, player_no='1')  # Loss pi before
        pi_l_old = pi_l_old.item()
        v_l_old = self.compute_loss_v(data, player_no='1').item()

        # Train policy for player 1 with multiple steps of gradient descent
        for i in range(self.train_pi_iters):
            self.pi_optimizer.zero_grad()
            loss_pi, pi_info = self.compute_loss_pi(data, player_no='1')
            kl = mpi_avg(pi_info['kl'])
            if kl > 1.5 * self.target_kl:
                self.logger.log('Early stopping 1st player at step %d due to reaching max kl.' % i)
                break
            loss_pi.backward()
        ...
        # Value function learning for player 1
        ...
        # Log changes from update for player 1
        ...
        # Player 2 updates
        if self.second_player_learning:
            # Compute initial loss and other metrics for player 2
            pi_l_old_2, pi_info_old_2 = self.compute_loss_pi(data, player_no='2')  # Loss pi before
            pi_l_old_2 = pi_l_old_2.item()
            v_l_old_2 = self.compute_loss_v(data, player_no='2').item()

            # Train policy for player 2 with multiple steps of gradient descent
            for i in range(self.train_pi_iters):
                self.pi_optimizer_2.zero_grad()
                loss_pi_2, pi_info_2 = self.compute_loss_pi(data, player_no='2')
                kl_2 = mpi_avg(pi_info_2['kl'])
                if kl_2 > 1.5 * self.target_kl:
                    self.logger.log('Early stopping 2nd player at step %d due to reaching max kl.' % i)
                    break
                loss_pi_2.backward()
            ...
            # Value function learning for player 2
            for i in range(self.train_v_iters):
                self.vf_optimizer_2.zero_grad()
                loss_v_2 = self.compute_loss_v(data, player_no='2')
                loss_v_2.backward()
                mpi_avg_grads(self.ac_2.v)  # average grads across MPI processes
                self.vf_optimizer_2.step()


            # Log changes from update for player 2
            kl_2, ent_2, cf_2 = pi_info_2['kl'], pi_info_old_2['ent'], pi_info_2['cf']
            self.logger.store(LossPi_2=pi_l_old_2, LossV_2=v_l_old_2,
                              KL_2=kl_2, Entropy_2=ent_2, ClipFrac_2=cf_2,
                              DeltaLossPi_2=(loss_pi_2.item() - pi_l_old_2),
                              DeltaLossV_2=(loss_v_2.item() - v_l_old_2))
        else:
            print('Second player is not learning')
            kl_2, ent_2, cf_2 = 0, 0, 0
            self.logger.store(LossPi_2=0, LossV_2=0,
                              KL_2=0, Entropy_2=0, ClipFrac_2=0,
                              DeltaLossPi_2=0,
                              DeltaLossV_2=0)
    ...
    def train(self, epochs=2000):
        ...
        for epoch in range(epochs):
            if self.second_player and self.random_learning:
                if np.random.rand() > 0.5:
                    self.second_player_learning = True
                    self.env.second_player = self.second_player_learning
                    print(colorize('Second player is learning', color='green', bold=True))
                else:
                    self.second_player_learning = False
                    self.env.second_player = self.second_player_learning
                    print(colorize('Second player is not learning', color='red', bold=True))
            ...
            for t in range(self.local_steps_per_epoch):
                ...
                # todo: fix crucial bug
                # a, _, _ = DDPG.ac.step(torch.as_tensor(o, dtype=torch.float32))
            ...
            # variable learnig rate
            # if epoch > 500: # todo: uncomment
                # self.pi_optimizer = Adam(self.ac.pi.parameters(), lr=3e-8)
                # self.vf_optimizer = Adam(self.ac.v.parameters(),  lr=1e-7)
                # print(colorize("Changed learning rate! 😜", 'cyan', bold=True))
            # if epoch == 500:
            #     self.env.final_range = self.env.final_range * 0.99
            #     self.env.error_range = self.env.error_range * 0.99
            if epoch >= 1000 and not epoch % 5 and self.env.final_range > 0.0025: # todo crucial bug
                self.env.final_range = self.env.final_range * 0.999
                self.env.error_range = self.env.error_range * 0.999
                print(epoch >= 1000, not epoch % 5, self.env.final_range > 0.0025)
            print(colorize(f"Error and final range is: {self.env.final_range}", 'cyan', bold=True))
    ...
    def test(self, plot=True, fun_mode=False, deterministic=True, save_data=False, parameter_variation=False,
        ...
        if save_data:
            ...
            # merge 2 data
            action_state = pd.concat([state_df, action_df], axis=1)
            # save
            action_state.to_csv('results/action_state.csv', index=False)
    ...
    # save second player
    def save_2(self, filepath='model/actor_critic_torch_2.pth'):
        if not os.path.exists('model/'):
            os.makedirs('model/')
        torch.save({
        ...
        print(colorize(f"2th Model saved to {filepath} 🔥🪐🥺🤭", 'green'))

    # load actor critic
    def load(self, filepath='model/actor_critic_torch_DG.pth'):
    ...
    def load_2(self, filepath='model/actor_critic_torch_2.pth'):
        ...
        print(colorize(f"2th Model loaded from {filepath} 🔥😎😌🥰", 'green'))
...
#
# DDPG = DDPG(ThreeBodyEnv(trajectory), ac_kwargs=dict(hidden_sizes=[HID] * L), gamma=GAMMA,
#             seed=SEED, steps_per_epoch=STEPS, logger_kwargs=logger_kwargs)
#
# DDPG.load()
# DDPG.env.error_range = 0.004
# DDPG.env.final_range = 0.004
#
# # test DDPG agent (not trained)
# DDPG.test(deterministic=True)
# #%%
# DDPG.test(deterministic=True, save_data=True)
# #%%
# # DDPG.second_player = True
# # DDPG.second_player_learning = True
# # DDPG.env.second_player = True
# #%%
# # DDPG.train(epochs=2000)
# #%%
# # DDPG.save()
# # DDPG.save_2()
# #%%
...
# #%%
# # fun_mode, deterministic = False, True
# # o, _ = DDPG.env.reset()
# # states = []
# # actions = []
# # actions_2 = []
# # while True:
# #     a, _, _ = DDPG.ac.step(torch.as_tensor(o, dtype=torch.float32), deterministic=deterministic)
# #     print(a)
# #     a_2, _, _ = DDPG.ac_2.step(torch.as_tensor(o, dtype=torch.float32), deterministic=deterministic)
# #     actions.append(a*DDPG.env.action_space.high[0])
# #     actions_2.append(a_2*DDPG.env.action_space.high[0])
# #     action = np.array([a.item(), a_2.item()])
# #     o, _, d, _, _, _ = DDPG.env.step(action)
# #     states.append(o*(DDPG.env.observation_space.high[0]-DDPG.env.observation_space.low[0])+DDPG.env.observation_space.low[0])
# #     if d:
# #         break
# # dt = DDPG.env.dt
