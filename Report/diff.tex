...
%DIF DEL DDPG-DG_old.tex   Tue Feb 11 16:51:31 2025
%DIF ADD DDPG-DG.tex       Tue Feb 18 14:39:51 2025
...
\author{\DIFdelbegin %DIFDELCMD < \IEEEauthorblockN{Hadi Nobahari}
%DIFDELCMD < \IEEEauthorblockA{\textit{Department of Aerospace Engineering} \\
%DIFDELCMD < \textit{Sharif University of Technology}\\
%DIFDELCMD < Tehran, Iran \\
%DIFDELCMD < nobahari@sharif.edu}
%DIFDELCMD < \and
%DIFDELCMD < %%%
\DIFdelend 
...
\DIFaddend \IEEEauthorblockA{\textit{Department of Aerospace Engineering} \\
...
This study proposes a reinforcement learning-based differential game framework for designing an adaptive, closed-loop controller for low-thrust spacecraft guidance. Unlike conventional methods, this approach does not require an explicit analytical model of the system's dynamics. Instead, it learns directly from \DIFaddbegin \DIFadd{the }\DIFaddend nonlinear motion equations, enabling a data-driven control strategy.
A neural network-based controller is trained using the Deep Deterministic Policy Gradient Differential Game (DDPG-DG) algorithm to generate real-time low-thrust control commands efficiently. The method is validated through \DIFdelbegin \DIFdel{simulated }\DIFdelend \DIFaddbegin \DIFadd{simulations of }\DIFaddend Lyapunov orbit transfers in the Earth-Moon system, where it demonstrates strong robustness to disturbances, engine imperfections, and environmental variations. The results highlight the potential of reinforcement learning-based differential game strategies to enhance spacecraft autonomy in complex gravitational environments.
\end{abstract}

\begin{IEEEkeywords}
Reinforcement \DIFdelbegin \DIFdel{Learning, Differential Game, Robust Controller, Low-Thrust, Multi-Body Dynamics
}\DIFdelend \DIFaddbegin \DIFadd{learning, Differential games, Robust control, Low-thrust propulsion, Multi-body dynamics
...
Machine learning is increasingly integral to space autonomy. Mars rovers employ ML for autonomous target selection (e.g., AEGIS on Curiosity~\cite{17}), reducing ground dependency for tasks like image analysis~\cite{18}. Future applications include autonomous robotics on Perseverance~\cite{19,20} and anomaly detection for Europa Clipper~\cite{Clipper}. RL-driven neural controllers \DIFdelbegin \DIFdel{further advance this trend by prioritizing }\DIFdelend \DIFaddbegin \DIFadd{prioritize }\DIFaddend feasibility over optimality, enabling rapid in-flight \DIFdelbegin \DIFdel{recomputation of control histories through }\DIFdelend \DIFaddbegin \DIFadd{re-computation of control commands via }\DIFaddend lightweight linear algebra operations\DIFaddbegin \DIFadd{, thus reducing reliance on iterative optimization}\DIFaddend . This approach reduces reliance on iterative optimization, mitigates communication risks, and enhances adaptability, positioning RL as a cornerstone of next-generation autonomous space missions.

While deep reinforcement learning~\cite{newrl} offers promising solutions, traditional methods in spacecraft guidance often rely on simplified models or pre-computed trajectory databases, which are not suitable for real-time operations in multi-body environments. DDPG~\cite{DDPG} has emerged as a robust solution for continuous control tasks, as it does not require \DIFdelbegin \DIFdel{an explicit }\DIFdelend \DIFaddbegin \DIFadd{a pre-defined analytical }\DIFaddend model of the system\DIFdelbegin \DIFdel{and can handle high-dimensional, continuous action spaces}\DIFdelend . However, pure DDPG is limited when it comes to multi-agent dynamics or complex interactions in multi-body systems, where spacecraft motion is influenced by gravitational forces from multiple bodies.
...
\begin{algorithmic}[1]
    ...
    \REPEAT
        ...
        \STATE If $s'$ is terminal, reset environment state.
        \IF{it's time to update}
            \DIFdelbegin %DIFDELCMD < \FOR{however many updates}
%DIFDELCMD <                 %%%
\DIFdelend \DIFaddbegin \FOR{\(j\) in range (however many updates)}
...
The robustness and generalization capabilities of the guidance strategy are evaluated through a series of simulation experiments that introduce disturbances, non-ideal thrust engine models, and model parameter variations. The experiments are designed to test the guidance strategy's ability to adapt to changing conditions and generalize across different mission scenarios. The results demonstrate the robustness of the guidance strategy \DIFdelbegin \DIFdel{in the presence of }\DIFdelend \DIFaddbegin \DIFadd{against }\DIFaddend disturbances and model uncertainties, as well as its ability to generalize effectively across different mission scenarios and low-thrust engine \DIFdelbegin \DIFdel{models}\DIFdelend \DIFaddbegin \DIFadd{configurations}\DIFaddend . The guidance strategy exhibits strong performance \DIFdelbegin \DIFdel{in challenging environments compared }\DIFdelend \DIFaddbegin \DIFadd{relative }\DIFaddend to conventional targeting \DIFdelbegin \DIFdel{guidance methods , highlighting its potential for autonomous spacecraft navigation in complex dynamical systems.
}\DIFdelend \DIFaddbegin \DIFadd{methods in simulation studies.
}

\DIFaddend % results ploted rewards in box plot
...
boxplot prepared={
  ...
  upper whisker=689.640044
	  ...
	  \end{tikzpicture}
	  \caption{
		Comparison of the total rewards obtained by the DDPG and DDPG-DG algorithms in the presence of \DIFdelbeginFL \DIFdelFL{model }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{gravitational }\DIFaddendFL parameter \DIFdelbeginFL \DIFdelFL{variations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{variation}\DIFaddendFL .
	}\label{fig:parameter}
\end{figure}
